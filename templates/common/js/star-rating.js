...
(function (window, document, undefined) {
  ...
  const Plugin = function (selector, options) {
    ...
    for (let i = 0; i < this.selects.length; i++) {
      if (this.selects[i].tagName !== "SELECT" || this.selects[i][handle])
        continue;
  ...
  Widget.prototype = {
    ...
    buildLabelEl_() {
      ...
      this.textEl = this.insertSpanEl_(
        this.widgetEl,
        {
    ...
    buildWidgetEl_() {
      ...
      const widgetEl = this.insertSpanEl_(
        this.el,
        {
    ...
    changeTo_(index) {
      ...
      if (this.options_.showText) {
        this.textEl.textContent =
          index < 1
            ? this.options_.initialText
            : this.widgetEl.childNodes[index - 1].dataset.text;
      }
      this.current = index;
    },

    /** @return HTMLElement */
    createSpanEl_(attributes) {
      // object
    ...
    /** @return void */
    eventListener_(el, action, events) {
    ...
    getIndexFromEvent_(ev) {
      ...
      return Math.min(
        Math.ceil(
          direction[this.direction] / Math.round(widgetWidth / this.stars)
        ),
        this.stars
      );
    },
    ...
    getSelectedValue_() {
      return (
        parseInt(this.el.options[Math.max(this.el.selectedIndex, 0)].value) || 0
      );
    },

    /** @return void */
    handleEvents_(action) {
      // string
    ...
    /** @return void */
    insertSpanEl_(el, attributes, after) {
      // HTMLElement, object, bool
    ...
    onKeydown_(ev) {
      ...
      this.setValue_(
        Math.min(Math.max(this.getSelectedValue_() + increment, 0), this.stars)
      );
    ...
    onPointerdown_(ev) {
      ...
      if (
        this.current !== 0 &&
        parseFloat(this.selected) === index &&
        this.options_.clearable
      ) {
        this.onReset_();
        this.triggerChangeEvent_();
        return;
