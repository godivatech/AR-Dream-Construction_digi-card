...
const shareOnWhatsappButton = document.getElementById(
...
// Comment out service worker to avoid caching external content
...
const getFeedbacks = (cardId, page, limit) =>
  new Promise((resolve, reject) => {
    ...
    xhr.open(
      "GET",
      `/api/v1/feedback?cardId=${cardId}&limit=${limit}&page=${page}`
    );
    xhr.setRequestHeader("Content-type", "application/json");
    xhr.send();
  });
const renderFeedbacks = (feedbacksArr) => {
  ...
  const elements = feedbacksArr.map(
    (feedback) => `<div class="feedback-wrapper">
...
const getAndRenderFeedbacks = async (cardId, element) => {
  try {
    ...
    feedbacks = feedbacks.concat(response.list);
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (feedbackList) {
    ...
    document
      .getElementById("load-more-feedback-btn")
...
const downloadImage = (ele, cardId, productName, productUrl) => {
  ...
  xhr.open(
    "GET",
    `/api/v1/download-product-image?cardId=${encodeURIComponent(
      cardId
...
const productDownloadButtons = document.getElementsByClassName(
...
const directWhatsappShareButton = document.getElementById(
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  productContainers.forEach((productContainer, index) => {
    const slideshowContainer = productContainer.querySelector(
  ...
  document.querySelectorAll("[class^=space-]").forEach((element) => {
    ...
    element.style.paddingTop = `${
      element.className.match(/(?:^|\s)space-(\d+)(?:$|\s)/)[1]
    }px`;
  });
});

// Add smooth scrolling for footer menu links
